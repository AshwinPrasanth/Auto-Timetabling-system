import sys
import pandas as pd
from PyQt5.QtWidgets import QApplication, QMainWindow, QPushButton, QTableWidget, QTableWidgetItem, QComboBox, QVBoxLayout, QWidget
...
class AllocationSystem(QMainWindow):
    ...
    def save_allocation(self):
        for row, faculty in self.selected_faculties.items():
            ...
            current_number = self.faculty_df.loc[self.faculty_df['Names'] == faculty, 'Numbers'].values[0]
            self.faculty_df.loc[self.faculty_df['Names'] == faculty, 'Numbers'] = current_number - 1
